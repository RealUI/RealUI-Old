...
function IceBarElement.prototype:Enable()
	...
	if self.moduleSettings.myTagVersion < nibIceHUD.CurrTagVersion then
		...
		self.moduleSettings.myTagVersion = nibIceHUD.CurrTagVersion
	end
		...
		opts["textSettings"] =
		{
			...
			args = {
				...
				lowerTextString = {
					type = 'input',
					name = L["Lower Text"],
					desc = L["The lower text to display under this bar.\n\nNOTE: this text block does NOT support DogTags.\n\nRemember to press ENTER/Accept after filling out this box or it will not save."],
					hidden = true,
					get = function()
...
function IceBarElement.prototype:ConditionalSetupUpdate()
	...
	if nibIceHUD.IceCore:IsUpdateSubscribed(self) then
		return
	end
	...
	-- Subscribe to MyOnUpdates for non-specific modules
	if not(string.find(self.elementName, "zCT")) and (self.elementName ~= "Vengeance") then
		nibIceHUD.IceCore:RequestUpdates(self, self.MyOnUpdateFunc)
	end
end
...
function IceBarElement.prototype:CreateBackground()
	if not (self.frame) then
		self.frame = CreateFrame("Frame", "nibIceHUD_"..self.elementName, self.masterFrame)
	end
...
function IceBarElement.prototype:CreateTexts()
	self.frame.bottomUpperText = self:FontFactory(nil, self.frame.bottomUpperText, false)
	self.frame.bottomLowerText = self:FontFactory(nil, self.frame.bottomLowerText, true)

	self.frame.bottomUpperText:SetHeight(14)
	self.frame.bottomLowerText:SetHeight(16)
	...
	self.frame.bottomUpperText:SetPoint("TOP"..ownPoint , self.frame, "BOTTOM"..parentPoint, offx + 0.5, offy + 0.5)
	self.frame.bottomLowerText:SetPoint("TOP"..ownPoint , self.frame, "BOTTOM"..parentPoint, offx + 0.5, (offy - 14) + 0.5)
...
function IceBarElement.prototype:SetBarCoord(barFrame, scale, top)
	...
	if scale == 0 then
		...
		if nibIceHUD:xor(self.moduleSettings.reverse, top) then
			if self.moduleSettings.inverse == "INVERSE" then
				min_y = 1 - scale
				max_y = 1
...
function IceBarElement.prototype:SetScale(inScale, force, skipLerp)
	...
	self.CurrScale = nibIceHUD:Clamp(self.CurrScale, 0, 1)
...
function IceBarElement.prototype:CreateMarker(idx)
	...
	self.Markers[idx].bar:SetVertexColor(color.r, color.g, color.b, self.moduleSettings.markers[idx].alpha or self.alpha)
...
function IceBarElement.prototype:PositionMarker(idx, pos)
	...
	if (self.moduleSettings.inverse == "INVERSE") then
		...
		min_y = nibIceHUD:Clamp(pos, 0, 1)
		max_y = nibIceHUD:Clamp(pos+heightScale, 0, 1)
	elseif (self.moduleSettings.inverse == "EXPAND") then
		pos = pos + ((1-pos) * 0.5)
		heightScale = heightScale * 0.5
		offset_y = self.settings.barHeight * (pos - 0.5)
		min_y = nibIceHUD:Clamp(1-pos-(heightScale), 0, 1)
		max_y = nibIceHUD:Clamp(1-pos+(heightScale), 0, 1)
	else
		offset_y = (self.settings.barHeight * pos)
		min_y = nibIceHUD:Clamp(1-pos-heightScale, 0, 1)
		max_y = nibIceHUD:Clamp(1-pos, 0, 1)
	end
