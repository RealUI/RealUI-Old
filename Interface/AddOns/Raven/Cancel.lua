...
local inCombatBarTemplate = {
	...
	pulseStart = 0, pulseEnd = 0, flashExpiring = 0, flashTime = 0, anchorFrame = 0, anchorPoint = 0, anchorX = 0, anchorY = 0
}
...
function MOD:SetInCombatBar(enable, lock, list, opt)
	...
	if g.count > 0 then
		...
		for i = 1, g.count do
			...
			v.highlight:SetAllPoints(v.container); v.backdrop:SetAllPoints(v.container)
		end
...
function MOD:UpdateInCombatBarOverlays()
	...
	if g and g.opt and g.initialized then -- make sure have valid layout
		if not InCombatLockdown() then -- can change enable setting only if out of combat
			...
			elseif g.hidden then g.frame:Show(); g.hidden = false; for i = 1, g.count do g.secureButtons[i]:Show() end end
		end
		for i = 1, g.count do
			local v = g.overlays[i]
			...
			if buff then
				v.highlight:SetTexture(icon); v.highlight:Show(); v.backdrop:Show(); v.container:EnableMouse(false)
				...
				if flashing then v.container:SetAlpha(Nest_FlashAlpha(1, 1)) else v.container:SetAlpha(1) end
			else
