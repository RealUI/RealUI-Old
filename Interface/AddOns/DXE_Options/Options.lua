	...
	do
		...
		local AR_group = {
			...
			args = {
				...
				desc = {
					type = "description",
					name = L.options["Activates the automatic responder for whispers during boss encounters. Others can send \"dxestatus\" to you to receive a short summary of the current fight (boss name, time elapsed, how many raid members are alive). The auto respnder works for normal and Battle.net whispers."],
					order = 1300,
					width = "full",
				},
				enabled = {
		...
		do
			...
			function handler:TestAlert(info)
				...
				if info.type == "dropdown" then
					addon.Alerts:Dropdown(var,info.varname,10,5,stgs.sound,stgs.color1,stgs.color2,stgs.flashscreen,info.icon,stgs.audiocd)
				elseif info.type == "centerpopup" then
					addon.Alerts:CenterPopup(var,info.varname,10,5,stgs.sound,stgs.color1,stgs.color2,stgs.flashscreen,info.icon,stgs.audiocd)
