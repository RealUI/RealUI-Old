...
function Map:Skin()
	...
	local function FixSkin()
		...
		local function SetDropdownHeaderFont(r)
			...
			r:SetTextColor(unpack(nibRealUI.media.colors.red))
...
function Map:UpdateCoords()
	if not redStr then redStr = nibRealUI:ColorTableToStr(nibRealUI.media.colors.red) end
	...
	if x ~= 0 and y ~= 0 then
		self.coords.player:SetText(string.format("|cff%s%s: |cffffffff%s, %s|r", redStr, PLAYER, x, y))
	else
	...
	if (adjustedX >= 0  and adjustedY >= 0 and adjustedX <= 1 and adjustedY <= 1) then
		...
		self.coords.mouse:SetText(string.format("|cff%s%s: |cffffffff%s, %s|r", redStr, MOUSE_LABEL, adjustedX, adjustedY))
	else
