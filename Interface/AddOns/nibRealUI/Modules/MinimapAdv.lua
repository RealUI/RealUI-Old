...
local function GetOptions()
	if not options then options = {
		type = "group",
		name = "Minimap",
		desc = "Advanced Minimap designed for RealUI.",
		arg = MODNAME,
		childGroups = "tab",
		order = 1309,
		args = {
			...
			desc = {
				type = "description",
				name = "Advanced Minimap designed for RealUI.",
				fontSize = "medium",
				order = 20,
			},
			enabled = {
			...
			information = {
				...
				args = {
					minimapbuttons = {
						...
						desc = "Moves buttons attached to the Miniamp to underneath and shows them on mouse-over.",
...
function MinimapAdv:UpdateInfoPosition()
	...
	if Minimap:IsVisible() and (ExpandedState == 0) then
		...
		-- Zone Indicator
		if MMFrames.info.zoneIndicator.isNotNeutral then
...
local function UpdateMMButtonsTable()
	for i, child in ipairs({Minimap:GetChildren()}) do
		if not(BlackList[child:GetName()]) and not(child.mmStyled) then
...
function MinimapAdv:ZoneChange()
	...
	MMFrames.info.zoneIndicator.isNotNeutral = (pvpType ~= nil)
	if MMFrames.info.zoneIndicator.isNotNeutral then
		MMFrames.info.zoneIndicator:Show()
	else
		MMFrames.info.zoneIndicator:Hide()
...
function MinimapAdv:PLAYER_LOGIN()
	MMFrames.buttonframe.edge:SetTexture(unpack(ClassColor))
end
...
local function CreateFrames()
	...
	MMFrames.buttonframe:SetBackdrop({
		bgFile = [[Interface\AddOns\MinimapAdv\Media\Plain]], 
		edgeFile = [[Interface\AddOns\MinimapAdv\Media\Plain]], 
		edgeSize = 1, 
	})
	MMFrames.buttonframe:SetBackdropColor(0, 0, 0, nibRealUI.media.backgroundalpha)
	MMFrames.buttonframe:SetBackdropBorderColor(0, 0, 0)
	...
	MMFrames.buttonframe.tooltipIcon:SetVertexColor(unpack(nibRealUI.media.colors.red))
	MMFrames.buttonframe.tooltipIcon:Hide()
	...
	-- Info
	MMFrames.info.location = NewInfoFrame("MinimapAdv_Location", Minimap)
	MMFrames.info.coords = NewInfoFrame("MinimapAdv_Coords", Minimap)
	MMFrames.info.coords:SetAlpha(0.75)
	MMFrames.info.dungeondifficulty = NewInfoFrame("MinimapAdv_DungeonDifficulty", Minimap)	
	MMFrames.info.queue = NewInfoFrame("MinimapAdv_Queue", Minimap)	
	MMFrames.info.RFqueue = NewInfoFrame("MinimapAdv_RFQueue", Minimap)	
	MMFrames.info.Squeue = NewInfoFrame("MinimapAdv_SQueue", Minimap)	
...
function MinimapAdv:OnEnable()
	ClassColor = nibRealUI:GetClassColor(nibRealUI.class)
