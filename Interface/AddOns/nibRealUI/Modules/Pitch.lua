			...
			animation = {
				...
				args = {
					updatespeed = {
						...
						get = function(info) return db.animation.updatespeed end,
						set = function(info, value) 
							db.animation.updatespeed = value
						end,
					},
					fadetime = {
						type = "range",
...
function Pitch:UpdateColors()
	for i, v in pairs(PitchElements) do
		...
		PitchFrame[v].bg:SetVertexColor(color[1], color[2], color[3], db.appearance.opacity.background)
